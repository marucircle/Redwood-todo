  ...
  scenario('creates a tag', async (scenario: StandardScenario) => {
    const result = await createTag({
      input: {
        ...
        bg_color: 'String',
        text_color: 'String',
        user_id: scenario.tag.two.user_id,
      },
    })

    expect(result.name).toEqual('String')
    expect(result.bg_color).toEqual('String')
    expect(result.text_color).toEqual('String')
    expect(result.user_id).toEqual(scenario.tag.two.user_id)
  })
